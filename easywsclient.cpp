...
class _RealWebSocket : public easywsclient::WebSocket
{
  public:
    ...
    bool useMask;
	HANDLE	write_buffer_mutex;
	void *callback_arg;

    _RealWebSocket(socket_t sockfd, bool useMask, void *arg) : sockfd(sockfd), readyState(OPEN), useMask(useMask) {
		write_buffer_mutex = CreateMutex(NULL, FALSE, NULL);
		callback_arg = arg;
    }

	~_RealWebSocket()
	{
		CloseHandle(write_buffer_mutex);
	}
    ...
    void poll(int timeout) { // timeout in milliseconds
		...
		WaitForSingleObject(write_buffer_mutex, INFINITE);
        while (txbuf.size()) {
		...
		ReleaseMutex(write_buffer_mutex);
    ...
    virtual void _dispatch(Callback_Imp & callable) {
            ...
            // Adapt void(const std::string<uint8_t>&) to void(const std::string&)
        {
            ...
            void operator()(const std::vector<uint8_t>& message, void* ptr) {
                std::string stringMessage(message.begin(), message.end());
                callable(stringMessage, ptr);
            }
        };
    ...
    virtual void _dispatchBinary(BytesCallback_Imp & callable) {
        ...
        while (true) {
            ...
            if (false) { }
                ...
                if (ws.fin) {
                    callable((const std::vector<uint8_t>) receivedData, callback_arg);
...
easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin, void *arg) {
    char host[128];
    int port;
    char path[128];
    if (url.size() >= 128) {
    ...
    return easywsclient::WebSocket::pointer(new _RealWebSocket(sockfd, useMask, arg));
}
...
WebSocket::pointer WebSocket::from_url(const std::string& url, const std::string& origin, void *arg) {
    return ::from_url(url, true, origin, arg);
}

WebSocket::pointer WebSocket::from_url_no_mask(const std::string& url, const std::string& origin, void *arg) {
    return ::from_url(url, false, origin, arg);
}
